<template>
  <div class="epidemic-right__block--2">
    <EpidemicToolBarBlock>
      <template #title> 教师核酸检测结果 </template>
      <template #content>
    ...
    <EpidemicToolBarBlock>
      <template #title> 教师接种新冠疫苗情况 </template>
      <template #content>
    ...
    <EpidemicToolBarBlock>
      <template #title> 教师跨区流动统计 </template>
...
<script>
import {
  apiGetTeacherNucleicAcid,
  apiGetTeacherVaccines,
  apiGetTeacherCross,
  apiAreaGetTeacherCross,
...
export default {
  ...
  methods: {
    decideTeacherCrossApi() {
      // 由于历史原因，关于教师跨区流动统计信息，全市和各区分为了两个不同的接口，所以需要多做一步判断
      if (this.addressId === '') {
        // 全市接口
        return apiGetTeacherCross();
      } else {
        // 各区接口
        return apiAreaGetTeacherCross(this.addressId);
      }
    },
    updateData() {
      ...
      Promise.all([
        apiGetTeacherNucleicAcid(this.addressId),
        apiGetTeacherVaccines(this.addressId),
        this.decideTeacherCrossApi(),
      ])
        .then(
          ([
            { result: nucleicAcidRes },
            { result: injectionRes },
            { result: TeacherCrossRes },
          ]) => {
            this.nucleicAcidRes = nucleicAcidRes;
            this.injectionRes = injectionRes;
            this.updateAcidBar(nucleicAcidRes);
            this.updatTeacherCrossBar(TeacherCrossRes);
          }
        )
        .catch((err) => {})
