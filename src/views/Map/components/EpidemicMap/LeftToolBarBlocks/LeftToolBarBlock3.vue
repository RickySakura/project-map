      ...
      <template #title-extra>
        <div class="table-type-switch">
          <span @click="setTableType(1)" :class="{ active: tableType == 1 }"
            >学校涉疫</span
          >
          <span @click="setTableType(2)" :class="{ active: tableType == 2 }"
            >学生涉疫</span
          >
        </div>
      </template>
...
import {
  ...
  apiGetRelatedStudentStatistics,
  ...
  methods: {
    updateData() {
      ...
      Promise.all([
        ...
        .then(([{ result: schoolRes }, { result: studentRes }]) => {
          ...
          this.tableRes.studentRes = mapper(studentRes, "student");
          this.tableData = this.tableRes.schoolRes;
        })
        .catch((err) => {
          console.error(
            "❌ ~ file: ToolBarBlock3.vue:77 ~ updateData ~ err",
            err
          );
        })
    ...
    setTableType(type) {
      this.tableType = type;
      this.tableData =
        type === 1 ? this.tableRes.schoolRes : this.tableRes.studentRes;
    },
  },
};
</script>
